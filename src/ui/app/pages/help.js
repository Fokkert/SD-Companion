...
${topic('start', '1. Initial setup', '<ol>' +
        ...
        '<li>Create any named schedules you need. A rule may alternatively use Always On.</li>' +
...
${topic('servers', '2. Jira servers', '<p>Each Server represents one Jira instance. Credentials, discovery scope, metadata, request pacing, health, capabilities and profiles are isolated per server.</p>' +
          ...
          '<li>The connection-loss alarm uses the active profile\'s configured alarm from Settings → Automation and can trigger by outage duration, consecutive failed health checks, or either threshold.</li>' +
...
${topic('discovery', '3. API discovery', '<p>Discovery uses Jira REST API data and transactional snapshots. An incomplete or cancelled deep sync does not replace the last successful inventory.</p>' +
            ...
            '<li>API Data categories are: Projects (project directory), Filters (saved filters), Users (assignable users), Issue Types (project issue types), Statuses (workflow statuses), Transitions (workflow-context transitions), Fields (visible Jira fields), Priorities (shared priorities), and Resolutions (shared resolutions).</li>' +
...
${topic('monitor', '4. Monitoring and Scan Now', '<p>Monitoring is the continuous rule-evaluation switch for the selected profile. While it is on, enabled rules are evaluated at the configured polling cadence and according to each rule\'s schedule. Scan Now performs one immediate cycle without changing Monitoring.</p>' +
              '<p>Home separates current detections from recent history and also shows completed, pending and failed actions by issue.</p>')}
${topic('rules', '5. Rule model', '<p>A rule defines what should match and what should happen.</p>' +
                ...
                '<li>Schedule is explicit: Always On or one or more selected named schedules. When several schedules are mapped to a rule, the rule is eligible while any selected schedule is active.</li>' +
                '<li>A saved filter is optional. Rules can use raw JQL, typed conditions, or both.</li>' +
...
${topic('trouble', '17. Troubleshooting', '<ul>' +
                                      ...
                                      '<li>To run queued work early, expand Home → Issue Action History and use Process beside a Pending action, or Process all for the issue/profile. To cancel, use Cancel beside the Pending/Running action. Cancelling one action does not automatically cancel later independent actions already queued for the same issue.</li>' +
                                      '</ul>')}
</section>`;
  };
})();
