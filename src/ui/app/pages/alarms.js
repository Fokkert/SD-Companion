(() => {
  ...
  const profileEditor = (alarm, isDefault) => {
    ...
    return `<div class="card editor-card alarm-profile-editor">` +
      ...
      `<div id="alarmStopMethodFields" class="alarm-stop-method-fields">${alarm.stopMethod === 'keyboard' ? `<div class="field"><label>Keyboard shortcut</label><input id="alarmKeyboardShortcut" class="input" value="${A.esc(alarm.keyboardShortcut || '')}" placeholder="Ctrl+Shift+S"></div>` : ''}</div>` +
      `<div class="field"><label>Duration</label><div class="duration-control"><input id="alarmDuration" class="input" type="number" min="1" step="1" value="${Number(alarm.durationSeconds) > 0 ? A.esc(SD.Utils.timeFromSeconds(alarm.durationSeconds, durationUnit)) : ''}" placeholder="Unlimited"><select id="alarmDurationUnit" class="select">${units(durationUnit)}</select></div></div>` +
