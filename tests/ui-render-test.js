...
assert.equal((fieldData.match(/Summary/g) || []).length, 1);
...
A.settingsAutomationSection = 'alarm';
automation = A.pageSettings();
assert(automation.includes('Profile alarm'));
assert(automation.includes('alarmVolumeValue'));
assert(!automation.includes('Alarm Studio'));
